function [problem,guess] = DoubleIntegrator
%DoubleIntergratorTracking - Double Integrator 
...
problem.states.x0=[0 2.5];

% Initial conditions for system. Bounds if x0 is free s.t. x0l=< x0 <=x0u
problem.states.x0l=[0 2.5]; 
problem.states.x0u=[0 2.5]; 



% State bounds. xl=< x <=xu
problem.states.xl=[0 -10];
problem.states.xu=[inf 10];

% State error bounds
problem.states.xErrorTol=[1e-4 1e-4];

% State constraint error bounds
problem.states.xConstraintTol=[1e-4 1e-4];

% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[0 -10];
problem.states.xfu=[inf 10];

% Guess the state trajectories with [x0 xf]
guess.time=[0 2 4 6 8 10];
guess.states(:,1)=[0 5 8 11 14 17];
guess.states(:,2)=[5 3 3 3 3 3];
...
problem.data.plantmodel = 'BangBangPlant';
...
%e2=x(:,2)-2.*cos(t); %Changement du magnitud             ... Il marche!
%e2=x(:,2)-3.*sin(t); %Changement du operation            ... Il marche!
%e2=x(:,1)-3; %Changement du etat avec reference constant... Il marche!


%e2=x(:,2)-0.3.*ones(size(t,1),1); % Reference constant avec "ones"... Il marche PAS!
e2=x(:,1)-10; %Reference constant numerique                ... Il marche PAS!
                     %Nota: les deux marchent bien entre -0.6 et 0.6
stageCost = e2.*e2;
%------------- END OF CODE --------------
