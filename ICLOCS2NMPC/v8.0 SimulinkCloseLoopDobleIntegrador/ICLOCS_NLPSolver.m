...
global infoNLP data solution N opt_t_min tstep Um1
persistent solution_new

simtime=tpx0(1); %Current simulation time (global)
Um1 = tpx0(2);
x0=tpx0(3:end);  %Current state

% Fetch the problem definition
[problem,guess]=DoubleIntegratorNMPC;          
...
if (simtime~=0 && ~re_opt) || mod(simtime,tstep)~=0 % For time instances without re-optimization
    ...
    % Return the solution
    if simtime~=0
        solution_new.computation_time=tstep; %Update at every time step
