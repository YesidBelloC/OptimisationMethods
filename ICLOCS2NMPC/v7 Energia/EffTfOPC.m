function [problem,guess] = EffTfOPC
...
x3max=72*45*3600;
problem.states.xl=[0 -10 0];
problem.states.xu=[inf 30 x3max];

% State error bounds
problem.states.xErrorTol=[1e-3 1e-3 1e-3];

% State constraint error bounds
problem.states.xConstraintTol=[1e-3 1e-3 1e-3];

% Terminal state bounds. xfl=< xf <=xfu
problem.states.xfl=[0 0 0]; 
problem.states.xfu=[inf 30 x3max];

% Guess the state trajectories with [x0 xf]
guess.states(:,1)=[0 1000];
guess.states(:,2)=[0 30];
guess.states(:,3)=[0 2e4];
...
problem.inputs.url=[-1e50]; 
problem.inputs.uru=[1e50]; 

% Input constraint error bounds
problem.inputs.uConstraintTol=[1e-2];
problem.inputs.urConstraintTol=[1e-20]; 
...
eff_k=(95.7926 - 1.3149e-4*(RPM-157.6525).^2 - 0.0013*(abs(T)-73.2008).^2);

%stageCost = e1.*e1+5e-4*(u(:,1).*u(:,1));   %El min u ayuda a min du, Eff
%tambn
stageCost = -1*eff_k + 1*e1.*e1 + 5e-4*(u(:,1).*u(:,1));
%stageCost = -eff_k + 5e-4*(u(:,1).*u(:,1));
%stageCost = 0*t;
%------------- END OF CODE --------------
...
RPM=Vx./(Rwr*0.10472);
eff_k=(95.7926 - 1.3149e-4*(RPM-157.6525).^2 - 0.0013*(abs(T)-73.2008).^2)/100;
