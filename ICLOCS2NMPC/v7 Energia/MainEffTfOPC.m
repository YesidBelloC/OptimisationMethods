...
[problem,guess]=EffTfOPC;                   % Fetch the problem definition
...
if (strcmp(options.transcription,'globalLGR')) || (strcmp(options.transcription,'hpLGR'))
    ...
    grid on
    subplot(3,1,3)
    plot([solution.T(:,1); solution.tf],speval(solution.Xp,3,solution.TSeg_Bar,[solution.T(:,1); solution.tf]),'b' )
    xlabel('Time [s]')
    ylabel('Energy [Ws]')
    grid on
    ...
    hold on
    plot(Speed(1:ceil(solution.tf))/3.6+2)
    %ylim([-1 max(speval(solution.Xp,2,solution.T(:,1)))+1])
    %xlim([0 solution.tf])
    xlabel('Time [s]')
    ylabel('Velocity [m/s]')
    grid on
    subplot(3,1,3)
    plot(solution.T(:,1),speval(solution.Xp,3,solution.T(:,1)),'b')
    xlabel('Time [s]')
    ylabel('Energy [Ws]')
    grid on
